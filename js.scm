...
(define (scm-obj? obj)
  (string=? "G_"
            (substring (class-of obj)
                       0
                       2)))

(define (get-slots obj)
  (if (usable-obj? obj)
      (##inline-host-expression "(@1@).slots;"
                                obj)
      (undefined-obj)))
...
(define (html-obj? obj)
  (if (and (scm-obj? obj)
           (has-slots? obj))
      (let ((first-slot (##inline-host-expression "(@1@)[0];"
                                                 (get-slots obj))))
        (if (has-slots? first-slot)
            (let ((sec-level-slots (get-slots first-slot)))
              (if (>= (##inline-host-expression "g_host2scm((@1@).length);"
                                                sec-level-slots)
                      3)
                  (let ((obj-symbol (##inline-host-expression "(@1@)[2];"
                                                             sec-level-slots)))
                    (let ((obj-symbol-name (##inline-host-expression "(@1@).name;"
                                                                   obj-symbol)))
...
(define (type-of obj)
  (js->scm (##inline-host-expression "typeof (@1@);"
                                           obj)))
...
(define (dom-obj? obj)
  (if (not (scm-obj? obj))
      (js->scm (##inline-host-expression "(@1@) instanceof Node;"
                                               obj))
      #f))
...
(define (query-selector selector
                       #!optional
                       (frame (document-obj)))
  (if (and (scm-obj? selector)
           (string? selector)
           (or (dom-obj? frame)
               (document-obj? frame)))
...
(define (query-selector-all selector
                          #!optional
                          (frame (document-obj)))
  (if (and (scm-obj? selector)
           (string? selector)
...
(define (get-attribute obj att)
  (if (and (dom-obj? obj)
           ...
           (string? att))
      (js->scm (##inline-host-expression "(@1@).getAttribute(@2@);"
                                               obj
                                               (scm->js att)))
      #f))
...
(define (get-element-type node)
  (if (not (scm-obj? node))
      (js->scm (##inline-host-expression "(@1@).nodeName;"
                                               node))
      #f))

(define (element-of-type? node type)
  (if (and (not (scm-obj? node))
           (string? type))
...
(define (first-child obj)
  (if (dom-obj? obj)
      (let ((first-given (##inline-host-expression "(@1@).firstChild;"
                                                  obj)))
        (if (or (null? first-given)
                (dom-obj? first-given)
                (and (element-of-type? first-given "#text")
                     (or (> (js->scm (##inline-host-expression "(@1@).length;"
                                                                     first-given))
                            1)
                         (not (string=? "\n"
                                        (js->scm (##inline-host-expression "(@1@).wholeText;"
                                                                                 first-given)))))))
            first-given
            (query-selector "*" obj)))
      #f))
  ...
  (let ((parent-temp (get-new-node))
    ...
    (if (set-inner-html parent-temp
        ...
        (let ((node-temp (first-child parent-temp)))
          (if node-temp
              (let ((append-returned (append-node parent
                                                node-temp)))
                (begin
...
(define (insert-html-before parent child-html ref-child)
  (let ((parent-temp (get-new-node))
    ...
    (if (set-inner-html parent-temp
        ...
        (let ((node-temp (first-child parent-temp)))
          (if node-temp
              (let ((insert-returned (insert-node-before parent
                                                      node-temp
                                                      ref-child)))
                (begin
...
(define (element-present-by-query? selector
                                 #!optional
                                 (frame (document-obj)))
  (not (null? (query-selector selector
                             frame))))
...
(define (replace-html old new-html)
  (let ((parent-temp (get-new-node))
    ...
    (if (set-inner-html parent-temp
        ...
        (let ((node-temp (first-child parent-temp)))
          (if node-temp
              (let ((replace-returned (replace-node old
                                                  node-temp)))
                (begin
    ...
    (let ((my-new-p (<p> id: my-id
      ...
      (begin
        ...
        (console.log (string-append "--- with invalid scm string\n"
                                    "    expect : null"))
        (console.log (get-element-by-id (string-append "invalid"
                                                    my-id)))
...
(define (test-get-attribute)
  (let ((my-id "myInput")
        ...
        (my-custom-val 11.1))
    (let ((my-new-input (<input> type: 'text
                               id: my-id
                               name: my-id
                               class: my-classes
                               value: my-value)))
